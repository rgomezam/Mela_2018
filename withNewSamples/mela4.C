...
//  ggTo4mu xsec = 0.00158549 pb
  ...
  char DiscriminantMVA[400] = {"p_JJVBF_BKG_MCFM_JECNominal/(p_JJVBF_BKG_MCFM_JECNominal+0.2*p_JJQCD_BKG_MCFM_JECNominal) >> h1"};
...
// ZZjj_MVA
  ...
  cout << "cutting MVA " << cuttingMVA << endl;
    sig->Draw("ZZjj_MVA >> h1",cuttingMVA); 
    float effsigMVA = h1->Integral()*normSig;    
    
  cout << "test effsigMVA " << effsigMVA << endl;
    ...
    effbkgMVA =+ h1->Integral()*normBkg3;
     ...
     cout << "test effbkgMVA " << effbkgMVA << endl;

    Tp[i] = effsigMVA;
    Zp[i] = effbkgMVA;


    cout << " Z, T (MVA) " << Zp[i] << "," << Tp[i] << endl; 
...
cout << "Seff MVA, Beff MVA" << SeffMVA[i] << "," << BeffMVA[i] << endl;
}
...
//Third and fourth: find the point given by the cuts: // (This has already been tested so I comment one of both) 
    ...
    sig->Draw(Discriminant,cuttingCandC);
    float effsig2 = h1->Integral()*normSig;
   
    
    bkg->Draw(Discriminant,cuttingCandC);
    float effbkg2 = h1->Integral()*normbkg;
 

    bkg1->Draw(Discriminant,cuttingCandC);
    effbkg2 =+ h1->Integral()*normBkg1;

    bkg2->Draw(Discriminant,cuttingCandC);
    effbkg2 =+ h1->Integral()*normBkg2;
 
    bkg3->Draw(Discriminant,cuttingCandC);
...
/*    sig->Draw("ZZjj_MVA >> h1",cuttingCandC); 
    ...
    cout << "test effsigMVA2" << effsigMVA2 << endl;   
...
//Test
TGraph *gr6 = new TGraph(ncuts,SeffMVA,BeffMVA); 

 
   
    gr1->SetLineWidth(2);
    gr1->SetLineColor(kRed);
    gr1->GetXaxis()->SetLimits(-0.07,1.01);
    gr1->SetMinimum(-0.01);
    gr1->SetMaximum(1.01);
    mg->Add(gr1); 
    ...
    gr3->GetXaxis()->SetLimits(-0.07,1.01);
    gr3->SetMinimum(-0.01);
    gr3->SetMaximum(1.01); 
    mg->Add(gr3); 
    ...
    gr6->GetXaxis()->SetLimits(-0.07,1.01);
    gr6->SetMinimum(-0.01);
    gr6->SetMaximum(1.01); 
    mg->Add(gr6); 
  ...
  legend->AddEntry(gr2, "Cut-based" , "p");
  legend->Draw("same");
